...
export default function TabBar({ navigationData, router }: TabBarProps) {
    ...
    return (
      <nav className="fixed flex flex-row items-center justify-between visible h-16 w-full px-8 text-2xl bg-[#fff] border-t-4 rounded-t-xl border-[#a4c852] z-50 bottom-0 md:hidden  md:invisible">
        {navigationData.map((item, index) => (
          <Link key={index} href={new URL(window.location.origin+item)}>
            <div className=" container bg-transparent hover:scale-125 ease-in-out duration-500 flex items-center justify-center h-10 w-10  ">
              <span
                className={`
                ${router.pathname == item.toString() ? "border-2 p-0.5 rounded-lg border-green-400  bg-[#a4c852]  " : ""}`
              }>
