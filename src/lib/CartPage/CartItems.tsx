...
export default function CartItems() {
  const { items, isEmpty, getItem, removeItem, addItem, updateItemQuantity } =
    useCart();
  ...
  if (isEmpty) {
    return (
      <div className="text-xl p-4 m-4">К сожалению, в корзине ничего нет </div>
    );
  } else {
    return (
      <>
                          ...
                          <div className="flex border border-black flex-row h-10 w-full rounded-md relative mt-1">
                            <button
                              onClick={() =>
                                updateItemQuantity(
                                  product.id.toString(),
                                  getItem(product.id.toString()).quantity - 1
                                )
                              }
                            ...
                            <input
                              ...
                              value={getItem(product.id.toString()).quantity}
                            ></input>
                            <button
                              onClick={() => {
                                addItem({
